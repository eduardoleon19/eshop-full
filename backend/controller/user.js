...
router.post("/login-user", catchAsyncErrors(async(req, res, next) => {
    try {
        ...
        console.log("user.comparePassword(password)", isPasswordValid)
...
router.put("/update-avatar", isAuthenticated, upload.single("image"), catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        const previousUser = await User.findByIdAndUpdate(req.user.id, {
            avatar: fileUrl,
        })        
        
        const existsUser = previousUser
...
router.put("/update-user-addresses", isAuthenticated, catchAsyncErrors(async(req, res, next) => {
    try {
        ...
        const existsAddresss = user.addresses.find(address => address._id === req.body._id)

        if (existsAddresss) {
            Object.assign(existsAddresss, req.body)
        } else {
...
router.delete(
    ...
    catchAsyncErrors(async (req, res, next) => {
        try {
            ...
            {_id: req.user._id}, { $pull: { addresses: { _id: req.params.id } } });
